...
kernel void convolution2d_direct(
    ...
    for (int j = 0; j < kernelHeight; j++) {
        for (int i = 0; i < kernelWidth; i++) {
            ...
            if (inputX >= 0 && inputX < inputWidth && inputY >= 0 && inputY < inputHeight) {
                sum += input[inputY * inputWidth + inputX] * h[(kernelHeight - 1 - j) * kernelWidth + (kernelWidth - 1 - i)];
            }
        }
    }
...
// ---------------------------------------------------------------------------
// The host performs 2D FFT convolution by orchestrating these kernels:
...
// Call with stage = 1, 2, ..., logN. Launch with global size (N/2, numRows).
kernel void fft_butterfly_rows(
    ...
    float2 u = data[idx1];
    float2 d = data[idx2];
    float2 v = (float2)(d.x * twiddle.x - d.y * twiddle.y,
                        d.x * twiddle.y + d.y * twiddle.x);
...
// Call with stage = 1, 2, ..., logN. Launch with global size (numCols, N/2).
kernel void fft_butterfly_cols(
    __global float2* data,
    const int stage,
    const int inverse,
    const int W
...
kernel void extract_and_normalize(
